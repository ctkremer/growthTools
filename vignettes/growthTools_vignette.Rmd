...
Now, we can use the function `get.nbcurve.tpc()` to fit thermal performance curves (using the re-parameterized Norberg function - see `?nbcurve()`) to the growth rate data. 

### Single curves:

This is easily done for a single thermal performancecurve, based on data collected over one dilution period:
...
More detailed information that can be directly accessed, including:
* TPC model type (Norberg or Double Exponential, currently)
* Parameter estimates for the TPC model
* Estimates of cardinal temperature traits (Topt, Tmin, Tmax, maximum growth rate)
...
Cardinal thermal traits will be the same across all TPC models we consider, where they exist and can be estimated. However, the number and identity of model parameters varies with the TPC equation under consideration. That is why model parameters are contained in the vector `$cf` within the `tpc` object.
...
The `predict()` function is useful for another primary objective, which is to visualize a fit thermal performance curve. This is handled by a plot method available for 'tpc' objects, which visualizes the fit thermal performance curve, and can also show the underlying observations and an approximate confidence band around this curve. This can be  a good way to visually assess the quality of the fit. The default plot looks like this:

```{r,error=FALSE,warning=F}
plot(nbcurve.fit)
```

These plots can be customized in various ways (see later section on Visualize Thermal Performance Curves). 
...
Or we can extract a table of values across multiple curves, using `dplyr` techniques:
...
Note that although the class of the results of `get.decurve.tpc` and `get.nbcurve.tpc` is the same (`tpc`), some of the contents of the resulting object differ. For example:
...
We could also apply this general approach to a larger data set, with more species and additional grouping variables:
...
Finally, it is possible to ammend these multi-curve plots with the underlying observations used as the foundation of the regressions. This takes advantage of the fact that the original observations are reported in the `data` field of `tpc` objects.
...
Microbial growth rate can also vary with the availability (concentration) of essential nutrients, such as 
nitrogen or phosphorus. The growthTools package includes a growing set of routines for fitting canonical
functions to these relationships (which we term 'nutrient performanc curves'), such as the Monod curve 
(Monod 1942). The syntax is very similar to that used for fitting thermal performance curves.
...
We can fit a Monod function to these data using 'mle2'. Note that currently a grid.mle2 method is not
...
As with thermal performance curves, there are built-in methods for printing summary information from the fit,
as well as displaying a plot of the fitted relationship.
...
```{r}
fit.monod.2<-get.monod(nutrients=dat$nutrients,mu=dat$mu,method='mle2',fix_intercept = F)
print(fit.monod)
print(fit.monod.2)
plot(fit.monod.2)
```
...
Again, as with thermal performance curves, the `get.monod` function works with tools from the `tidyverse` so we
can fit multiple data sets at once. For example:
...
res <- monod.tmp %>% group_by(sps) %>% 
  do(npcs=get.monod(.$nutrients,.$mu,method='mle2'))
